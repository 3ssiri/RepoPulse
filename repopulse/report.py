...
# Shown when the file listing was cut short (local max-files cap or GitHub tree
# API limit): the score covers only the files that were listed.
TRUNCATION_NOTE = (
    "File listing was truncated (repository too large for a full listing); checks ran on a "
    "partial file list and results may be incomplete."
)
...
def render_markdown(report: HealthReport) -> str:
    ...
    if report.scan_truncated:
        lines.extend([f"> ⚠️ **Truncated scan:** {TRUNCATION_NOTE}", ""])
    lines += [
        "## Repository",
...
def render_summary(report: HealthReport) -> str:
    ...
    if report.scan_truncated:
        lines.append(f"Warning: {TRUNCATION_NOTE}")
    if report.recommendations:
...
def render_terminal(report: HealthReport, console: Console | None = None, verbose: bool = False) -> None:
    ...
    if report.scan_truncated:
        target.print(f"[bold yellow]Warning:[/bold yellow] [yellow]{TRUNCATION_NOTE}[/yellow]")
